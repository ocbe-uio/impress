# Code to make the ran dataset using a sham randomisation.
# Used until the real randomisation is available
#



make_shamrand <- function(raw, cfg) {
  ...
  ran0 <- dm %>% 
    ...
    mutate(randia = factor(randia, levels = levels(ran$randia)),
           ...
           ranst = factor(paste0(ranst, " (Sham)")),
           ranfudos = factor(ranfudos, levels = levels(ran$ranfudos))) %>%
    copy_labels_from(ran)
  
  if (cfg$mode == "sham"){
  raw <- raw %>% 
    mutate(txt = if_else(cfg$mode == "sham" & id == "ran", list(ran0), txt),
           data = if_else(cfg$mode == "sham" & id == "ran", list(ran0), data),
           data_lbl = if_else(cfg$mode == "sham" & id == "ran", list(ran0), data_lbl
           ))
  }
